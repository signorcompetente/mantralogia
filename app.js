...
function FantacalcioSwitch() {
  ...
  className="px-5 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-bold text-3xl shadow-xl hover:scale-105 hover:shadow-2xl transition-all duration-200"
...
const calcolaFMVMedia = () => {
  ...
  Object.entries(fmvPerRuolo).forEach(([r, valori]) => {
    const top8 = valori.sort((a, b) => b - a).slice(0, 8);
    media[r] = top8.reduce((s, v) => s + v, 0) / top8.length;
  });
  return media;
};

const fmvMediaRuoli = calcolaFMVMedia();
...
const calcolaPrioritaEmergenti = () => {
    ...
    top3.forEach((mod, idx) => {
      ...
      Object.entries(richiestiModulo).forEach(([r, richiesti]) => {
        ...
        const fmvMod = fmvMediaRuoli[r] ? Math.log(fmvMediaRuoli[r] + 1) / 3 : 1;
const pesoRuolo = (vincolanti[r] || 1) * fmvMod;
...
{titolari.length >= 3 && (porteAperte.length > 0 || fuoriRuoloAvvisi.length > 0) && (
  <div className="fixed top-4 left-4 z-50 w-80 rounded-2xl shadow-2xl overflow-hidden" style={{fontFamily: "'Courier New', monospace", background: '#fffef0', border: '2px solid #d4c97a', boxShadow: '4px 4px 12px rgba(0,0,0,0.2)'}}>
    {/* Spirale in cima */}
